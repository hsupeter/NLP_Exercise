    ...
    def _build_graph(self, name='build_graph'):
        ...
        with tf.name_scope(name='create_embeddings'):
            ...
            for i in range(2*self.window_size):
                embedding_i = tf.nn.embedding_lookup(self.embeddings,
                                 train_dataset[:,i])        
            ...
            with tf.name_scope(name='valid_dataset'):
                ...
                mean_batch_embedding = tf.reduce_mean(tf.nn.embedding_lookup(
                        self.embeddings, test_labels),axis=0)
...
def fit(self, text_digt, reverse_dictionary, test_text_digt,
    ...
    with self._session.as_default() as sess:                               
        ...
        for step in range(num_steps):            
            ...
            gnBh = genBatch()            
            train_batch, train_labels, trn_idx = gnBh.train_batch_cbow(
                    text_digt, train_batch_size=batch_size,
        ...
        for k,v in test_text_digt.items():
            ...
            for test_step in range(num_test_steps):
                test_batch_labels = gnBh.test_batch(test_text_digt[k],
                                    batch_size, test_idx = test_idx)
...
test_batch(test_text_digt
        [list(test_text_digt.keys())[0]], test_batch_size=8,
        test_idx = test_idx)
