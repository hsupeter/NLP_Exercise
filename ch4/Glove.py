...
### Defining the Model Computations
 ...
 data points (Xij) and co-occurence matrix weights (Xij).
 Xij measures the importance of a data point with 
 respect to how much those two words co-occur and Xij denotes
...
embed_in = tf.nn.embedding_lookup(in_embeddings, train_input)
...
Xij = tf.placeholder(tf.float32,shape=[batch_size],\
                           name='Xij') 
...
loss = tf.reduce_mean(\
  Xij * (tf.reduce_sum(embed_in*embed_out,axis=1)\
  +embed_bias_in +embed_bias_out -tf.log(epsilon+Xij))**2)
...
with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as session:
    ...
    for step in range(num_steps):
        ...
        batch_input, batch_labels, batch_weights =\
        ...
        # train_input/train_labels: training inputs and training labels
        # Xij: measures the importance of a data point with respect to how much those two words co-occur
        # Xij: co-occurence matrix value for the row and column denoted by the words in a datapoint
        feed_dict = {train_input : batch_input.reshape(-1),\
                train_labels : batch_labels.reshape(-1),\
                Xij:batch_weights, Xij:batch_xij}
