...
train_dataset = tf.placeholder(tf.float32,
                   [None, num_unroll, n_inputs],
                   name='train_dataset')  
train_labels = tf.placeholder(tf.float32,
                   [None, num_unroll, n_outputs],
                   name='train_labels')

'''!!!!! Test List !!!!!
tf.unstack axis=1把train_labels原本tensor的[None, num_unroll, n_outputs]
降維成[None(train_batchSize), n_outputs]，length為num_unroll的list

train_labels_list = tf.unstack(train_labels, axis=1)
!!!!! Test End !!!!!'''
...
train_outputs, states = tf.nn.dynamic_rnn(cell,
                    train_dataset, dtype = tf.float32)
trainScores = tf.layers.dense(train_outputs, n_outputs)

trainPredictions = tf.nn.softmax(trainScores)

train_loss = tf.reduce_mean(
             tf.nn.softmax_cross_entropy_with_logits_v2(
             logits=trainScores, labels=train_labels)) #純量

train_perplexity_without_exp = tf.reduce_sum(
        (train_labels) * - tf.log(trainPredictions+1e-10)        
...
gradients, v = zip(*rnn_optimizer.compute_gradients(train_loss))
...
valid_dataset = tf.placeholder(tf.float32,
    [valid_batchSize,valid_num_unroll,n_inputs],name='valid_dataset')
valid_labels = tf.placeholder(tf.float32,
    [valid_batchSize,valid_num_unroll,n_outputs],name='valid_labels')

valid_outputs, valid_states = tf.nn.dynamic_rnn(cell,
                    valid_dataset, dtype = tf.float32)
validScores = tf.layers.dense(valid_outputs, n_outputs)
validPredictions = tf.nn.softmax(validScores)

valid_loss = tf.reduce_mean(
             tf.nn.softmax_cross_entropy_with_logits_v2(
             logits=validScores, labels=valid_labels)) #純量

valid_perplexity_without_exp = tf.reduce_sum(
        valid_labels*-tf.log(validPredictions+1e-10))/(
        valid_num_unroll*valid_batchSize)
 
# In[9]:
predPhase_batchSize = 1
predPhase_num_unroll= 1
predPhase_dataset = tf.placeholder(tf.float32,
    [predPhase_batchSize, predPhase_num_unroll, n_inputs],
    name='predPhase_dataset')

predPhase_outputs, predPhase_states = tf.nn.dynamic_rnn(cell,
                    predPhase_dataset, dtype = tf.float32)

predPhaseScores = tf.layers.dense(predPhase_states, n_outputs)
#predPhaseScores = tf.layers.dense(predPhase_outputs, n_outputs)
predPhasePredictions = tf.nn.softmax(predPhaseScores)
...
for fi in range(num_files):
  ...
  if fi not in long_doc_ids:
    train_gens.append(DataGeneratorOHE(
            text_digt[fi], train_batchSize, num_unroll))
    ...
    if iteration % valid_often == 0:            
      ...
      for vld_doc_id in range(len(valid_gens)):  
          ...
          for vld_iteration in range(train_times_per_doc//2):              
            ...
            valid_dat = np.reshape(np.hstack(unroll_validXArray),
                            (-1, valid_num_unroll, n_inputs))
            unroll_validyArray = np.array(unroll_valid_y)
            valid_lbl = np.reshape(np.hstack(unroll_validyArray),
                            (-1, valid_num_unroll, n_outputs))
            
            vld_loss_val,vld_perplx = session.run([valid_loss,
                                 valid_perplexity_without_exp],
                                 feed_dict = {valid_dataset: valid_dat,
                                 valid_labels: valid_lbl})
      ...
      for _ in range(segments_to_generate):
        ...
        predPhase_word = np.zeros((predPhase_batchSize,
               predPhase_num_unroll, n_inputs),dtype=np.float32) # shape(1, 544)
        ...
        for _ in range(chars_in_segment):   # 預測chars_in_segment個字
          predPhase_pred = session.run(predPhasePredictions,
          ...
          predPhase_word = np.zeros((predPhase_batchSize,
               predPhase_num_unroll, n_inputs),dtype=np.float32)
