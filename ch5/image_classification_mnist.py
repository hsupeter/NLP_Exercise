...
# 
# Here we define the set of hyperparameters we're going to you in our example.
# These hyperparameters include `batch_size`, train dataset size
#  (`n_train`), different layers in our CNN (`cnn_layer_ids`).
#  You can find descriptions of each hyperparameter in comments.
...
layer_hyperparameters = {'conv1':{'weight_shape':[3,3,n_channels,16],
                                  'stride':[1,1,1,1],'padding':'SAME'},
                         'pool1':{'kernel_shape':[1,3,3,1],
                                  'stride':[1,2,2,1],'padding':'SAME'},
                         'conv2':{'weight_shape':[3,3,16,32],\
                                  'stride':[1,1,1,1],'padding':'SAME'},\
                         'pool2':{'kernel_shape':[1,3,3,1],\
                                  'stride':[1,2,2,1],'padding':'SAME'},\
...
# Here we define input and output placeholders required
# to process a batch of data. We will use the same 
# placeholders for all training, validation and testing
# data as all of them are processed in same size batches.

# In[3]:


# Inputs (Images) and Outputs (Labels) Placeholders
tf_inputs = tf.placeholder(shape=[batch_size, image_size, 
  image_size, n_channels],dtype=tf.float32,name='tf_mnist_images')
tf_labels = tf.placeholder(shape=[batch_size, n_classes],
                   dtype=tf.float32,name='tf_mnist_labels')


# ## Defining Model Parameters and Other Variables
# Here we define various TensorFlow variables required 
# for the following computations. These includes a global 
# step variable (to decay learning rate) and weights and 
# biases of each layer of the CNN.
...
for layer_id in cnn_layer_ids:
    if 'pool' not in layer_id:
        layer_weights[layer_id] = tf.Variable(initial_value=
             tf.random_normal(shape=layer_hyperparameters
             [layer_id]['weight_shape'],stddev=0.02,dtype=
             tf.float32), name=layer_id+'_weights')
        
        layer_biases[layer_id]=tf.Variable(initial_value = 
             tf.random_normal(shape=[layer_hyperparameters
             [layer_id]['weight_shape'][-1]], stddev=0.01,
             dtype=tf.float32),name=layer_id+'_bias')
    
print('Variables initialized')


# ## Defining Inference of the CNN
# Here we define the computations starting from input 
# placeholder (`tf_inputs`) and then computing the hidden 
# activations for each of the layers found in `cnn_layer_ids`
# (i.e. convolution/pooling and fulcon layers) and their 
# respective parameters (`layer_hyperparamters`). 
# At the final layer (`softmax`) we do not apply an activation
# function as for the rest of the layers, but obtain the 
# unnormalized logit values without any activation function.
...
for layer_id in cnn_layer_ids:
    if 'conv' in layer_id:
        # For each convolution layer, compute the output by
        # using conv2d function
        # This operation results in a [batch_size, output_height,
        # output_width, out_channels] sized 4 dimensional tensor
        
        h = tf.nn.conv2d(h,layer_weights[layer_id],
                         layer_hyperparameters[layer_id]['stride'],
                         layer_hyperparameters[layer_id]['padding'])\
                         +layer_biases[layer_id]
        h = tf.nn.relu(h)

    elif 'pool' in layer_id:
        # For each pooling layer, compute the output by max pooling
        # max pooling
        # This operation results in a [batch_size, 
        # output_height, output_width, out_channels]
        # sized 4 dimensional tensor
        h = tf.nn.max_pool(h, layer_hyperparameters[layer_id] 
            ['kernel_shape'], layer_hyperparameters[layer_id]
            ['stride'], layer_hyperparameters[layer_id]
            ['padding'])

    elif layer_id == 'fulcon1':
        # At the first fulcon layer we need to reshape 
        # the 4 dimensional output to a 2 dimensional 
        # output to be processed by fully connected layers
        # Note this should only done once, before 
        # computing the output of the first fulcon layer
        h = tf.reshape(h,[batch_size,-1])
        h = tf.matmul(h,layer_weights[layer_id]) +\
                layer_biases[layer_id]
        h = tf.nn.relu(h)

    elif layer_id == 'softmax':
        # Note that here we do not perform the same 
        # reshaping we did for fulcon1
        # We only perform the matrix multiplication on 
        # previous output
        h = tf.matmul(h,layer_weights[layer_id]) +\
                layer_biases[layer_id]
...
# ## Defining Loss
# We use softmax cross entropy loss to optimize 
# the parameters of the model.

# In[6]:


# Calculating the softmax cross entropy loss with the computed logits 
# and true labels (one hot encoded)
tf_loss = tf.nn.softmax_cross_entropy_with_logits_v2(
            logits=tf_logits,labels=tf_labels)
print('Loss defined')


# ## Model Parameter Optimizer
# We define an exponentially decaying learning rate and
# an optimizer to optimize the parameters.
...
tf_learning_rate = tf.train.exponential_decay(
        ...
        decay_rate=0.5,decay_steps=1,staircase=True)
tf_loss_minimize = tf.train.RMSPropOptimizer(learning_rate =
        tf_learning_rate, momentum=0.9).minimize(tf_loss)
print('Loss minimization defined')


# ## Defining Predictions
# We get the predictiosn out by applying a softmax 
# activation to the logits. Additionally we define 
# a global step increment function and will be increase 
# every time the validation accuracy plateus.
...
# ## Define Accuracy 
# A simple function to calculate accuracy for 
# a given set of labels and predictions.
...
def accuracy(predictions,labels):
    '''
    Accuracy of a given set of predictions of size 
     (N x n_classes) and abels of size (N x n_classes)
    '''
...
# ## Lolading Data
# 
# Here we download (if needed) the MNIST dataset and,
# perform reshaping and normalization. Also we conver  
# the labels to one hot encoded vectors.
...
def read_mnist(fname_img, fname_lbl, one_hot=False):
    print('\nReading files %s and %s'%(fname_img,
                                       fname_lbl))
    ...
    with gzip.open(fname_img) as fimg:        
        ...
        img = (np.frombuffer(fimg.read(num*rows*cols),
               dtype=np.uint8).reshape(num, rows, 
               cols,1)).astype(np.float32)
        print('(Images) Returned a tensor of shape ',
              img.shape)
    ...
    with gzip.open(fname_lbl) as flbl:
        ...
        if one_hot:
            one_hot_lbl = np.zeros(shape=(num,10),
                                   dtype=np.float32)
            one_hot_lbl[np.arange(num),lbl] = 1.0
        print('(Labels) Returned a tensor of shape: %s'
              %lbl.shape)
...
train_inputs, train_labels = read_mnist(
                        ...
                        'train-labels-idx1-ubyte.gz',True)
test_inputs, test_labels = read_mnist(
                         't10k-images-idx3-ubyte.gz',
                         't10k-labels-idx1-ubyte.gz',True)
valid_inputs, valid_labels = train_inputs[-n_valid:,:,:,:],\
                             train_labels[-n_valid:,:]
train_inputs, train_labels = train_inputs[:-n_valid,:,:,:],\
                             train_labels[:-n_valid,:]
...
# 
# Here we have the logic to iterate through each training,
# validation and testing datasets, in `batch_size` size strides.
...
def get_train_batch(images, labels, batch_size):
    global train_index
    batch = images[train_index:train_index+batch_size,:,:,:],\
            labels[train_index:train_index+batch_size,:]
...
def get_valid_batch(images, labels, batch_size):
    global valid_index
    batch = images[valid_index:valid_index+batch_size,:,:,:],\
            labels[valid_index:valid_index+batch_size,:]
    valid_index = (valid_index + batch_size)%(
                    images.shape[0] - batch_size)
    return batch

def get_test_batch(images, labels, batch_size):
    global test_index
    batch = images[test_index:test_index+batch_size,:,:,:],\
            labels[test_index:test_index+batch_size,:]
    test_index = (test_index + batch_size)%(
                  images.shape[0] - batch_size)
    return batch
...
correctly_predicted = np.empty(shape=(10,28,28,1),
                               dtype=np.float32)
correct_predictions = np.empty(shape=(10,n_classes),
                               dtype=np.float32)
incorrectly_predicted = np.empty(shape=(10,28,28,1),
                                 dtype=np.float32)
...
def collect_samples(test_batch_predictions,test_images,
    ...
    if correct_indices.size>0 and correct_fill_index<10:
        ...
        correctly_predicted[correct_fill_index,:,:,:] = \
    ...
    if incorrect_indices.size>0 and incorrect_fill_index < 10:
        ...
        incorrectly_predicted[incorrect_fill_index,:,:,:] = \
                    test_images[chosen_index,:].reshape(1, 
                    image_size, image_size, n_channels)
...
# Here we train our CNN on MNIST data for `n_epochs` epochs.
# Each epoch we train the CNN with the full training dataset.
# Then we calculate the validation accuracy, according to 
# which we decay the learning rate. Finally, each epoch we 
# calculate the test accuracy which is computed using an 
# independent test set. This code should run under  
# 10 minutes if you run on a decent GPU and should reach  
# to a test accuracy of about ~95%

# In[75]:
# Parameters related to learning rate decay
# counts how many times the validation accuracy 
# has not increased consecutively for
v_acc_not_increased_for = 0 
# if the above count is above this value, 
...
# Good practice to use this to avoid any surprising 
# errors thrown by TensorFlow
config.gpu_options.allow_growth = True
# Making sure Tensorflow doesn't overflow the GPU 
config.gpu_options.per_process_gpu_memory_fraction = 0.9 

n_epochs = 2 # Number of epochs the training runs for
...
for epoch in range(n_epochs):
    ...
    for i in range(n_valid//batch_size):
        ...
        vbatch_images,vbatch_labels = get_valid_batch(
                valid_inputs, valid_labels, batch_size)
        # Compute validation predictions
        valid_batch_predictions = session.run(
            tf_predictions,feed_dict={tf_inputs: vbatch_images}
            )
        # Compute and add the validation accuracy  
        # to a python list
        valid_accuracy_per_epoch.append(accuracy(
                valid_batch_predictions, vbatch_labels))
    ...
    for i in range(n_test//batch_size):
        btest_images, btest_labels = get_test_batch(test_inputs,
                                        test_labels, batch_size)
        test_batch_predictions = session.run(tf_predictions,
                             feed_dict={tf_inputs: btest_images})
        print('\ni:', i)
        print('test_batch_predictions:',
              test_batch_predictions[:1, :])
        accuracy_per_epoch.append(accuracy(test_batch_predictions,
                                           btest_labels))
        #print('accuracy_per_epoch shape',
        #       accuracy_per_epoch.shape)
        # Collect samples for visualization only in the last epoch
        if epoch==n_epochs-1:
            collect_samples(test_batch_predictions, btest_images,
                            btest_labels)
...
for sub_i in range(7):
    ...
    pylab.imshow(np.squeeze(correctly_predicted[sub_i]),
                 cmap='gray')    
    pylab.axis('off')
...
for sub_i in range(7):
    ...
    pylab.imshow(np.squeeze(incorrectly_predicted[sub_i]),
                 cmap='gray')
    pylab.axis('off')
    
    # Draw the fourth row (incorrect predictions bar chart)
    pylab.subplot(4, 7, 21 + sub_i + 1)        
    pylab.bar(x_axis + padding, incorrect_predictions[
            sub_i], width)
    pylab.ylim([0.0,1.0])
    pylab.xticks(x_axis, labels)
